\documentclass[11pt]{report}
% packages and definitions
\input{include/include.tex}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\auteur}{Hugo Brunet}
\newcommand{\articletitle}{Étude et application des séries temporelles pour les données fonctionnelles avec la prise en compte de la régularité des trajectoires}
\newcommand{\customdate}{Avril—Septembre 2023}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


\dominitoc
\begin{document}
%%%%%%%%%%%%%%%%%%%%%%%%%% INCLUDE COMMAND %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%% AND DEFAULTS PARAMETERS %%%%%%%%%%%%%%%%%%%%%%%%%
\input{include/commands.tex}
\input{include/default_params.tex}
%%%%%%%%%%%%%%%%%%%%%%%%% DEBUT COURVERTURE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{content/couverture/main.tex}
%%%%%%%%%%%%%%%%%%%%%%%%% FIN COURVERTURE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%% DEBUT ABSTRACT %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{content/abstract.tex}
%%%%%%%%%%%%%%%%%%%%%%%%% FIN ABSTRACT %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Ici on insère la table des matières (entre astract et les chapitres)
\tableofcontents
%%%%%%%%%%%%%%%%%%%%%%%%% DEBUT CHAPITRES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{content/chapter_1/main.tex}



\input{content/chapter_2/main.tex}
\input{content/chapter_3/main.tex}
%%%%%%%%%%%%%%%%%%%%%%%%% FIN CHAPITRES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

example citation \cite{pmle}
\typeout{}
\bibliography{bibliography/main}

\end{document}
